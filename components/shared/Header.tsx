...
const Links = [
  ...
  { label: "Favorites", href: "/liked" },
  { label: "My Orders", href: "/myOrders" },
  { label: "Information", href: "/info" },
]

const infoNames = ["Contact", "Shipping & Payment", "Warranty & Services"]
      ...
      <div className="w-full max-w-[1680px] h-full flex justify-between items-center bg-black px-5 max-[600px]:px-9 max-[500px]:px-7">
        ...
        <nav className="w-fit h-11 flex gap-1 justify-center items-center rounded-xl bg-zinc-800/50 px-2 max-lg:hidden">
          ...
          {Links.map(({ label, href }, index) => {
            ...
            if (["Favorites", "My Orders"].includes(label)) {
              ...
              return (
                <div key={label}>
                  ...
                  >
                    <TransitionLink
                      href={`${href}${label === "Favorites" ? "/" + userInfo?._id : ""}`}
